 ...
 * the TDC-7200 chip which is the time-of-flight (TOF) from
 * the event on the START to the count of the COARSE_CLOCK timer
 * that is latched in the PICstop variable.  TOF is *subtracted*
 * from (PICstop * PICTICK_PS) to generate the timestamp.
 *
 * In theory TOF could be up to 6 ms which in picoseconds would require 
 * a 64 bit var.  But the hardware design constrains the TOF range to between
 * 300,000 ps (300 ns) and 100,300,000 ps (100.3 us).  That fits comfortably
 * in a 32 bit var, and we want to use 32 bit vars wherever we can because
 * the Arduino is very slow at handling 64 bit vars.
 *
 * You will see in the main loop that we iterate through the two channels
 * looking for the INTB signal raised by the TDC7200 chip to indicate when
 * a measurement is ready.  The delay between PICstop being latched and INTB
 * asserting between 2330 ns and 2472 ns.  Because of this variable delay
 * and the fact that the two channels are checked for INTB sequentially,
 * if both channels are active there is no guarantee which one will be processed
 * first, and thus timestamps could be printed out of order or out of channel
 * sequence.  The pairing scheme discussed below mitigates this.
 *
 * Further fun facts: the Cursor AI agent tells me that the loop iteration time
