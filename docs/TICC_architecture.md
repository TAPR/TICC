...
**Date:** October 6, 2025  **Version:** 20251006.1

## Overview

This document describes the architecture and implementation details of the
TICC (Time Interval Counter) firmware. The TICC uses a TDC7200 chip to
provide precise timestamp measurements with picosecond accuracy.

## Hardware
The TDC7200 has only a few connections that we use:
- START (event begins)
- STOP  (event ends)
...
fed to the TDC STOP pin, and also to an ISR on the Arduino that copies the
current value of the PICcount timescale accumulator into a variable called
PICstop.  
...
Timestamps are generated by combining the captured PICstop value at the
instant the STOP signal hits the TDC7200 with the tof generated by the
TDC chip.  To be precise, since PICstop represents the coarse tick *after*
the START event, tof is *subtracted* from (PICstop * 100 us) to yield
the timestamp.

## Main Loop Processing
The main loop iterates through the two channels looking for the INTB signal
raised by the TDC7200 chip to indicate when a measurement is ready. The
delay between PICstop being latched and INTB asserting is between 2330 ns
and 2472 ns. Because of this variable delay and the fact that the two
channels are checked for INTB sequentially, if both channels are active
there is no guarantee which one will be processed first, and thus timestamps
could be printed out of order or out of channel sequence.

The code checking for INTB and processing timestamps is designed to correct
this by always outputting timestamps in strict ascending sequence.
